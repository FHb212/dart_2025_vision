...
int UsbcdcTransporter::read(unsigned char *buffer, size_t len) {
  int ReceiveSize = 0;
  if (libusb_bulk_transfer(device_handle, read_endpont_, buffer, len, &ReceiveSize, read_timeout_) <
      0) {
    return -1;
  }
  return ReceiveSize;
...
int UsbcdcTransporter::write(unsigned char *buffer, size_t len) {
  ...
  return libusb_bulk_transfer(device_handle, write_endpoint_, buffer, len, &ActualTransmitSize,
                              write_timeout_);
}
